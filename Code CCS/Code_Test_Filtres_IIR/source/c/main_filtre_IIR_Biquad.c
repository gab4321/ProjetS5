...
const int Fs = 8000;
int noCorde=2; // Numéro de la corde de guitare, permet de sélectionner le filtre désiré
...
int main()
{
    if(debug)
    {
        ...
        for(i = 10; i < 2020; i+=5)
        {
            //yy = filtrerCascadeIIR(noCorde, i);
            //pTampon = direct1FIR_ASM(pTampon, i, CoeffsFIR, &xFIR);
            pTampon = direct1FIR(pTampon, i, CoeffsFIR, FIRNbCOEFFS, tampon, TAMPON_L, &xFIR);
        }
        i = 1;
    }
    else
...
{
	static int inputInterne = 1, FIR_ASM = 0, FLT_IIR = 1;
	short x, xFIR, y, y1;
	static int n=1;
	...
	if (FLT_IIR)
	{
	    // FILTRAGE OCTAVE (523,25 Hz a 1046,50 Hz)
